...
(function () {
    ...
    // entering links.
    var linkDialogText = "<p><b>插入超链接</b></p><p>创建超链接。</p>";
    var imageDialogText = "<p><b>插入图片</b></p><p>选择本地的图片进行上传。</p>";
    var fileDialogText = "<p><b>插入文件</b></p><p>选择本地的文件进行上传。</p>";

    // The default text that appears in the dialog input box when entering
    // links.
    var imageDefaultText = "http://";
    var linkDefaultText = "http://";
    var fileDefaultText = "http://";
    ...
    //      was chosen).
    ui.prompt = function (text, defaultInputText, callback, type) {
        ...
        var createDialog = function () {
			...
			// 描述
            inputDesc = doc.createElement("input");
            inputDesc.type = "hidden";
            inputDesc.value = "描述";
            box.appendChild(inputDesc);
            	...
            	var img_uploader = new AjaxUpload(upload_img_btn,{
            		...
            		onComplete : function(file, response){
            			if(response.status == "success"){
							...
							//新增回调文件名
							inputDesc.value = response.fname;
            			}else{
            				alert("上传失败，请重试。");
            			}
            		}
			...
			if(type=="file"){
            	...
            	var img_uploader = new AjaxUpload(upload_img_btn,{
            		...
            		action : "/upload/image",
            		onSubmit: function(file,ext) {
            			if (! (ext && /^(doc|docx|ppt|pptx|pdf)$/i.test(ext))){
            				alert('只能上传 doc|docx|ppt|pptx|pdf 格式的文件。');
            				return false;
            			};
            		},
            		data: { user_action: 'upload_image'},
    ...
    function UIManager(postfix, panels, undoManager, previewManager, commandManager, helpOptions) {
        ...
        function makeSpritedButtonRow() {
			...
			buttons.file = makeButton("wmd-file-button", "附件 <img> Ctrl+F", "-260px", bindCommand(function (chunk, postProcessing) {
                return this.doLinkOrImage(chunk, postProcessing, "file");
            }));
			makeSpacer(2);
    ...
    commandProto.doLinkOrImage = function (chunk, postProcessing, type) {
        ...
        if (chunk.endTag.length > 1 && chunk.startTag.length > 0) {
            ...
            var linkEnteredCallback = function (link,desc) {
                ...
                if (link !== null) {
                    ...
                    chunk.startTag = (type=="image") ? "![" : "[";
                    chunk.endTag = "][" + num + "]";

                    if (!chunk.selection) {
                        if (type=="image") {
                            chunk.selection = desc;
                        }
                        else {
                            chunk.selection = desc;
                        }
            ...
            if (type=="image") {
                if (!this.hooks.insertImageDialog(linkEnteredCallback))
                    ui.prompt(imageDialogText, imageDefaultText, linkEnteredCallback, type);
            }            
			if (type=="file") {
                if (!this.hooks.insertImageDialog(linkEnteredCallback))
                    ui.prompt(fileDialogText, fileDefaultText, linkEnteredCallback, type);
            }
            if (type=="link") {
                ui.prompt(linkDialogText, linkDefaultText, linkEnteredCallback,type);
            }
            return true;
        }
    };
